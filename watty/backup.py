...
def backup(output: str = None, db_path: str = None) -> str:
    ...
    if not os.path.exists(db):
        print("[Watty] No brain.db found. Nothing to back up.", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Backed up to {output} ({manifest['memories']} memories)", flush=True)
    return output
...
def restore(archive: str, force: bool = False, db_path: str = None) -> dict:
    ...
    if not os.path.exists(archive):
        print(f"[Watty] Backup not found: {archive}", file=sys.stderr)
        sys.exit(1)

    with tarfile.open(archive, "r:gz") as tar:
        names = tar.getnames()
        if "manifest.json" not in names or "brain.db" not in names:
            print("[Watty] Invalid backup: missing manifest.json or brain.db", file=sys.stderr)
            sys.exit(1)

        manifest = json.loads(tar.extractfile("manifest.json").read())

        if os.path.exists(target_db) and not force:
            print(f"[Watty] brain.db already exists ({target_db}). Use --force to overwrite.", file=sys.stderr)
            sys.exit(1)

        if manifest.get("version") != SERVER_VERSION:
            print(f"[Watty] Warning: backup version {manifest.get('version')} != current {SERVER_VERSION}", file=sys.stderr)
    ...
    print(f"Restored: {manifest['memories']} memories, {len(manifest.get('providers', []))} providers", flush=True)
    return manifest
